  ...
  module LLM
    class GoogleVertexAI
      ...
      def authenticated?
        ...
        begin
          @authorizer.fetch_access_token!['access_token'].to_s.length == 1024
      ...
      def chat(params = {})
        ...
        if params[:tool_choice]
          params[:tool_choice] =
            { function_calling_config: { mode: params[:tool_choice].upcase } }
        end
        # chat_url = params.fetch(:url, url)

        raise ArgumentError, 'messages argument is required' if Array(params[:messages]).empty?
