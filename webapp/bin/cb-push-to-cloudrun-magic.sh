...
if [ -f /secretenvrc/gemini-news-crawler-envrc ] ; then
  ...
  echo 'Riccardo NOT Secret or it doesnt work with this weird path. to be sure, lets check.'
  ls -al /
  ls -al /secretenvrc
fi
...
case "$1" in
   dev | development )
    export APP_TO_DEPLOY="${APP_NAME}-dev"
    ;;

  prod | production )
    export APP_TO_DEPLOY="${APP_NAME}-prod"
    ;;
